...
NOX_Sprite_t *NOX_CreateSprite(SDL_Renderer *renderer, const char *file, SDL_Point frame_size)
{
	...
	if (sprite == NULL) {
		NOX_DisplayMessage(NOX_LOG_ERROR | NOX_LOG_FATAL, "Cannot Allocate Memory For Sprite");
		return NULL;
	}

	if (NOX_LoadSprite(renderer, &sprite, file) < 0) {
		NOX_DisplayMessage(NOX_LOG_ERROR, "Cannot Create Sprite Object");
		return NULL;
	}
	...
	NOX_DisplayMessage(NOX_LOG_INFO, "Sprite Object Created");
	return sprite;
}
...
int NOX_LoadSprite(SDL_Renderer *renderer, NOX_Sprite_t **sprite, const char *file)
{
	...
	if ((*sprite)->data == NULL) {
		NOX_DisplayMessage(NOX_LOG_ERROR, "Cannot Load Sprite Texture File: %s", file);
	...
	switch ((*sprite)->nr_channels) {
		...
		case STBI_rgb_alpha:
			...
			break;
		default:
			NOX_DisplayMessage(NOX_LOG_ERROR, "Cannot Identify Sprite Texture Pixel Format");
			NOX_DestroySprite(*sprite);
			return -1;
	}
	...
	if ((*sprite)->surface == NULL) {
		NOX_DisplayMessage(NOX_LOG_SDL2_ERROR, "Cannot Create Sprite Surface For Texture");
		NOX_DestroySprite(*sprite);
		return -1;
	}
	...
	if ((*sprite)->texture == NULL) {
		NOX_DisplayMessage(NOX_LOG_SDL2_ERROR, "Cannot Create Texture From Sprite Surface");
		NOX_DestroySprite(*sprite);
		return -1;
	}
...
void NOX_RenderSprite(SDL_Renderer *renderer, NOX_Sprite_t *sprite, SDL_FPoint position, SDL_FPoint size)
{
	...
	if (!sprite->can_draw || (renderer == NULL && sprite == NULL)) {
		NOX_DisplayMessage(NOX_LOG_ERROR, "Sprite Or Renderer Is NULL");
	...
	if (success < 0) {
		NOX_DisplayMessage(NOX_LOG_SDL2_ERROR, "Cannot Copy Sprite Texture To Renderer");
		sprite->can_draw = false;
		return;
	}
}
...
void NOX_DestroySprite(NOX_Sprite_t *sprite)
{
	if (sprite->texture != NULL) {
		...
		NOX_DisplayMessage(NOX_LOG_INFO, "Sprite Texture Destroyed");
	}

	if (sprite->surface != NULL) {
		SDL_FreeSurface(sprite->surface);
		NOX_DisplayMessage(NOX_LOG_INFO, "Sprite Texture Surface Freed");
	}

	if (sprite->data != NULL) {
		stbi_image_free(sprite->data);
		NOX_DisplayMessage(NOX_LOG_INFO, "Sprite Texture Data Freed");
	}

	if (sprite != NULL) {
		free(sprite);
		NOX_DisplayMessage(NOX_LOG_INFO, "Sprite Memory Freed");
	}
}
